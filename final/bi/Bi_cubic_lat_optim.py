...
def make_struc(alat):
    ...
    bi_crystal = crystal(symbols=['Bi'], basis=[(0.5, 0.5, 0.5)], spacegroup=221, cellpar=[alat, alat, alat, 90, 90, 90])
    structure = Struc(ase2struc(bi_crystal))
    return structure
...
def compute_energy(alat, nk, ecut):
    ...
    dirname = "Bi_bcc_alat_{}_ecut_{}_nk_{}".format(alat, ecut, nk)
    runpath = Dir(path=os.path.join(os.environ["WORKDIR"], "final-project/final/bi/cubic/lattice", dirname))
...
def lattice_scan():
    nk = 5
    ecut = 70
    alat_list = numpy.linspace(3.1, 3.3, 21)
    ...
    for alat in alat_list:
        ...
        energy_list.append(round(float(output["energy"] / 1), 5))
        print(output)

    with open('Bi_lattice_optimization.csv', mode='w', newline='') as file:
