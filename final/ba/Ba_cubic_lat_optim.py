...
def make_struc(alat):
    ...
    crystal = bulk('Ba', crystalstructure='bcc', a=alat)
    structure = Struc(ase2struc(crystal))
    return structure
...
def compute_energy(alat, nk, ecut):
    ...
    dirname = "Ba_bcc_alat_{}_ecut_{}_nk_{}".format(alat, ecut, nk)
    runpath = Dir(path=os.path.join(os.environ["WORKDIR"], "final-project/final/ba/cubic/lattice", dirname))
...
def lattice_scan():
    nk = 5
    ecut = 70
    alat_list = numpy.linspace(4.9, 5.1, 21)
    ...
    with open('Ba_lattice_optimization.csv', mode='w', newline='') as file:
